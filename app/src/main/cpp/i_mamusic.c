...
static musicinfo_t *channels_playing[NUM_CHANNELS];
...
static void AllocateSound(musicinfo_t *musicinfo, byte *data, size_t len, int sample_rate)
{
...
{
    musicinfo_t *musicinfo = channels_playing[channel];
    if (musicinfo == NULL)
        return;
...
static void GetMusicLumpName(musicinfo_t *music, char *buf, size_t buf_len)
{
    // No music links
    //    if (music->link != NULL)
    //       music = music->link;
    ...
    if (use_music_prefix)
        M_snprintf(buf, buf_len, "ds%s", music->name);
    else
        M_StringCopy(buf, music->name, buf_len);
}

static void I_MA_PrecacheSounds(musicinfo_t *sounds, int num_sounds)
{
    for (int i = 0; i < num_sounds; ++i)
    {
        musicinfo_t *musicinfo = &sounds[i];

        // need to load the sound
        char name[9];
        GetMusicLumpName(musicinfo, name, sizeof(name));
        musicinfo->lumpnum = W_CheckNumForName(name);
        //printf("music->name = %s", musicinfo->name);
        //printf("music->lumpnum = %d", musicinfo->lumpnum);
        int lumpnum = musicinfo->lumpnum;
        if (lumpnum == -1)
            continue;
        ...
        AllocateSound(musicinfo, data, length, samplerate);
...
static void I_MA_UpdateSoundParams(int handle, int vol, int sep)
{
    ...
    musicinfo_t *musicinfo = channels_playing[handle];
    if (musicinfo == NULL)
        return;

    allocated_sound_t *snd = musicinfo->data;
    if (snd == NULL)
        return;
...
// Retrieve the raw data lump index for a given music name.
static int I_MA_GetMusicLumpNum(musicinfo_t *music)
{
    char name[9];
    GetMusicLumpName(music, name, sizeof(name));
...
// As the Music info struct contains
...
static int I_MA_StartSound(musicinfo_t *musicinfo, int channel, int vol, int sep)
{
    ...
    channels_playing[channel] = musicinfo;
...
static boolean I_MA_InitSound(void)
{
